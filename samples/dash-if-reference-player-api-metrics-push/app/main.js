...
app.controller('DashController', function ($scope, sources, contributors, dashifTestVectors) {
    ...
    $scope.updateMetricsInterval = 5000;
    ...
    $scope.player.on(dashjs.MediaPlayer.events.STREAM_INITIALIZED, function (e) { /* jshint ignore:line */
        ...
        $scope.metricsTimer = setInterval(function () {
            const metric = {}
            if (!$scope.metricsArray) {
                $scope.metricsArray = [];
            }
            // console.log($scope['metricsArray']);
            metric['video'] = updateMetrics('video');
            metric['audio'] = updateMetrics('audio');
            metric['time'] = Math.floor(new Date() / 1000);
            $scope.metricsArray.push(metric);
            $scope.chartCount++;
    ...
    $scope.enableChartByName = function (id, type) {
        ...
        $scope.chartOptions.legend.noColumns = Math.min($scope.chartData.length, 5); TODO remove me*/
    };
    ...
    function updateMetrics(type) {
        ...
        if (dashMetrics && $scope.streamInfo) {
            ...
            innerMetric['bufferLevel'] = bufferLevel;
            innerMetric['maxIndex'] = maxIndex;
            innerMetric['bitrate'] = bitrate;
            innerMetric['droppedFrames'] = droppedFPS;
            innerMetric['liveLatency'] = liveLatency;
            ...
            if (httpMetrics) {
                ...
                innerMetric['download'] = parseHTTPMetric(httpMetrics.download[type]);
                innerMetric['latency'] = parseHTTPMetric(httpMetrics.latency[type]);
                innerMetric['ratio'] = parseHTTPMetric(httpMetrics.ratio[type]);
            }
            ...
            if ($scope.chartCount % 2 === 0) {
                ...
                TODO remove me
                */
                innerMetric['videoTime'] = time;
            }
            innerMetric['time'] = Math.floor(new Date() / 1000);
        }
        return innerMetric;
    }
    ...
    (function init() {
        ...
        if (vars && vars.hasOwnProperty('apiUrl')) {
            $scope.apiUrl = vars.apiUrl;
        }
